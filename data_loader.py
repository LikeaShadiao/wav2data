...
class data_generator():
    ...
    def __init__(self,train_dir = TRAIN_DIR, 
        ...
        if RIR_dir is not None:
            ...
            for idx, rir_item in enumerate(self.rir_list[:5]):
                print('Item at position {}: {}'.format(idx, rir_item))
    ...
    def find_files(self,file_name):
        ...
        # 改小一些
        Begin_S = int(np.random.uniform(0,5 - self.length_per_sample)) * self.fs
        Begin_N = int(np.random.uniform(0,5 - self.length_per_sample)) * self.fs
                ...
                if self.add_reverb:
                    if reverb_rate < self.reverb_rate:
                        ...
                        if len(rir_s.shape)>1:
                            rir_s = rir_s[:,0]
                        # 查看rir_s维度
                        print("-----rir_s dimention: ", rir_s.shape)
